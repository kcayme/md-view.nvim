...
describe("theme_switch", function()
  ...
  describe("explicit modes push palette SSE event", function()
    it("should push palette event with non-empty css for dark", function()
      active_previews = { make_preview() }
      M.set_theme("dark")
      -- dark/light/auto: palette + theme("") + hljs_theme (3 events)
      assert.are.equal(3, #sse_events)
      assert.are.equal("palette", sse_events[1].event)
      assert.truthy(sse_events[1].data.css and sse_events[1].data.css ~= "")
      assert.are.equal("theme", sse_events[2].event)
      assert.are.equal("", sse_events[2].data.css)
      assert.are.equal("hljs_theme", sse_events[3].event)
      assert.truthy(sse_events[3].data.url and sse_events[3].data.url ~= "")
    end)
    ...
    it("should push palette event with non-empty css for sync", function()
      active_previews = { make_preview() }
      M.set_theme("sync")
      -- sync: palette only (hljs uses inline CSS overrides, no URL or theme clear to push)
    ...
    it("should push palette event to all active previews", function()
      ...
      M.set_theme("dark")
      -- 2 previews × 3 events each (palette + theme + hljs_theme) = 6 total
      assert.are.equal(6, #sse_events)
    end)
  end)
