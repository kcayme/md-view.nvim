...
sequenceDiagram
    ...
    Neovim->>buffer: TextChanged / TextChangedI / BufWritePost<br/>BufReadPost / FileChangedShellPost
    Note over buffer: debounce timer resets (300ms default)
    ...
    Note over buffer: External write (e.g. AI agent) detected by fs_event watcher
    buffer->>buffer: fs debounce fires → read file from disk
    buffer->>preview: callbacks.on_content(lines)
    preview->>sse: push("content", {content})
    sse->>Browser: event: content\ndata: {json}
```
...
sequenceDiagram
    ...
    User->>init: :MdViewStop / BufDelete / BufWipeout / VimLeavePre
...
sequenceDiagram
    ...
    preview->>preview: init_hub() → start hub server (port H)
    preview->>hub: register(bufnr=1, path, tab_label)
    preview->>hub: push("preview_added", {id=1, title, label})
    ...
    Browser->>hub: GET /sse
    Note over hub: add_client replay order:<br/>1. hub_palette / hub_hljs_theme / hub_theme (chrome styled first)<br/>2. preview_added → palette → theme per preview (panel created + styled)<br/>3. on_client_added → async disk read → content
    hub-->>Browser: SSE: hub_palette {css}
    hub-->>Browser: SSE: preview_added {id=1, title, label}
...
sequenceDiagram
    ...
    preview->>hub: push("preview_added", {id=2, title, label})
    preview->>hub: push("hub_palette", {css})
    preview->>buffer: watch(bufnr=2, callbacks)
    hub-->>Browser: SSE: preview_added {id=2, title, label}
